...
if (isset($_POST['accept'])) {
    ...
    $sql = "UPDATE Demande SET isValidated = 1, dateReponse = CURRENT_DATE WHERE id = $demandId";
...
if (isset($_POST['decline'])) {
    $demandId = $_POST['demand_id'];
    $sql = "UPDATE Demande SET isValidated = 0, dateReponse = CURRENT_DATE WHERE id = $demandId";
    $conn->query($sql);
}

if (isset($_POST['download'])) {
    // Handle Download file with name like "Document$id.pdf"
    $demandId = $_POST['demand_id'];
    ...
    header('Content-Disposition: attachment; filename="Document' . $demandId . '.pdf"');
    header('Expires: 0');
